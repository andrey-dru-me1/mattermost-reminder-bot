...
2. `reminder` - сервер с основной логикой бота, который обрабатывает слеш-команды из Mattermost, а также предоставляет полезные API-эндпоинты для работы с напоминалками
   1. `MYSQL_USER`
   2. `MYSQL_PASSWORD`
   3. `DB_HOST`
   4. `DB_PORT`
